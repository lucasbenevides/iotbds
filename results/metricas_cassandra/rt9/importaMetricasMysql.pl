		...
		while ($linha = <ARQ>){
			...
			foreach $campo (keys %hashCampos){
				...
				if ($hashCampos{$campo} =~ m/^[0-9]*\.?[0-9]+/){ #eh Float
					$sql = "INSERT INTO cassandra_metricas(server, timestamp, metric, type, value, value_f) VALUES (?, ?, ?, ?, ?, ?)";
					$stmt = $myConnection->prepare($sql);
					$stmt->execute($server, $timestamp, $metrica, $campo, $hashCampos{$campo}, $hashCampos{$campo}) or die "Erro ao inserir registro com valor Float - $campo -> $hashCampos{$campo}\n";
				}
				else {
					$sql = "INSERT INTO cassandra_metricas(server, timestamp, metric, type, value) VALUES (?, ?, ?, ?, ?)";
					$stmt = $myConnection->prepare($sql);
					$stmt->execute($server, $timestamp, $metrica, $campo, $hashCampos{$campo}) or die "Problema ao inserir registro sem Float";
				}
			}
		}
		close ARQ;
